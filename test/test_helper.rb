...
module TestHelpers
  ...
  def create_payload
    ...
    payload = PayloadRequest.create(url_id: url.id, requested_at: "abcd", responded_in: "5",
  ...
  def create_multiple_payloads(number)
    number.times do |i|
      ...
      payload = PayloadRequest.create(url_id: url.id, requested_at: "abcd#{i}", responded_in: "#{i}",
                                      referrer_id: referrer.id, request_type_id: rtype.id,
                                      user_agent_id: user_agent.id, resolution_id: resolution.id,
                                      ip_address_id: ip.id)
    end
